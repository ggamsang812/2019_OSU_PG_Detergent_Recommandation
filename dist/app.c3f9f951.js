...
xhttp.onreadystatechange = function () {
  if (this.readyState == 4 && this.status == 200) {
    response = JSON.parse(xhttp.responseText); //console.log(response.choices);
  }
};
...
function check() {
  ...
  if (count >= 4) {
    ...
    window.location.href = '/weightpage.html';
  }
}
...
function picked(userchoice, index) {
  ...
  if (userchoice.style.background != 'rgb(246, 174, 45)') {
    ...
    console.log(response.choices[index].value); //push the index of the images clicked

    y.push(index);
  } else {
    response.choices[index].value = false;
    userchoice.style.background = '#006FB6';
    console.log(response.choices[index].value); //pop the Index
